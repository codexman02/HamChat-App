...
let contactData:{contact_id:string,username:string,lastMessage:{content:string,sentAt:Date},numberOfNewMessages:number}={
...
async function sendAudio(){
  ...
  if(recorder){
    ...
    recorder.ondataavailable=(e)=>{
      ...
      //     image_id:"2345"
      //   },
      //   sentBy:"choco@gmail.com",
      //   sentAt:Date.now().toLocaleString(),
      //   seenBy:["choco@gmail.com"]
...
function showOptions(id:string){
  // setInDots(true);
...
function hideOptions(id:string){
  ...
  // console.log(ele,"options hidden")
}

function pekka(val:boolean,id?:string){
  cheeku=val
  // console.log("pekka",cheeku)
  // hideOptions(id)
}
               ...
               {ele.sentBy == user.email ? (
                 <div className="w-full p-3 relative flex justify-end" key={i}>
                  {/* <div className={`absolute options_${i} top-4 right-7 transition-all duration-300 hidden bg-white px-5 shadow-md py-2 rounded-xl  gap-3 border border-solid border-gray-200`} onMouseEnter={()=>{pekka(true)}} onMouseLeave={()=>{pekka(false,`options_${i}`)}} >
                    ...
                    <span className="inline-block w-1/2" onMouseEnter={(e)=>{showOptions(`options_${i}`)}} onMouseLeave={()=>{hideOptions(`options_${i}`)}}><MessageComponent ele={ele} same={true} index={i} modal={openModal}/></span>
                 ...
                 <div
                   ...
                   key={i}
                 >
                   <span className="inline-block w-1/2" onMouseEnter={(e)=>{showOptions(`options_${i}`)}} onMouseLeave={()=>{hideOptions(`options_${i}`)}}>
